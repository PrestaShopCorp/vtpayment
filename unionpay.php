...
class UnionPay extends PaymentModule
{
	private $html = '';
	...
	public function __construct()
	{
		...
		$arr = array('PAYMENT_URL', 'MERCHANT_ID', 'PAY_TYPE', 'PAY_METHOD', 'LANGUAGE', 'INITIAL_ORDER_STATUS_ID', 'SECURE_HASH_SECRET');
		...
		$this->description = 'UnionPay Online Payment Module.';
	...
	//	$remark				= $cart_id;
		...
		$http_orhttps = 'http';
		$success_url = $http_orhttps.'://'.$_SERVER['HTTP_HOST'].__PS_BASE_URI__.'modules/unionpay/validation.php?success=true&cart_id='.$cart_id;
		$acq_id = '99020344';
		$array = array(
	...
	foreach ($md5_array as $k => $v)
		...
		//MD5加密
		return md5($md5str.Configuration::get('SECURE_HASH_SECRET'));
	}
	public function verifyPaymentDatafeed($src, $prc, $success_code, $merchant_reference_number,
	$unionpaydollar_reference_number, $currency_code, $amount, $payer_authentication_status, $secure_hash_secret, $secure_hash)
	{
		$buffer = $src.'|'.$prc.'|'.$success_code.'|'.$merchant_reference_number.'|'.
		$unionpaydollar_reference_number.'|'.$currency_code.'|'.$amount.'|'.$payer_authentication_status.'|'.$secure_hash_secret;
	...
	$secure_hash_secret = Tools::getValue('SECURE_HASH_SECRET');
	if ($secure_hash_secret === false)
	$this->post_errors[] = 'SECURE_HASH_SECRET is required.';
	...
	private function postProcess()
	{
		if ((Tools::getValue('btnSubmit')))
		{
			...
			if (Tools::getValue('SECURE_HASH_SECRET'))
				...
				$secure_hash_secret = '';
			Configuration::updateValue('SECURE_HASH_SECRET', $secure_hash_secret);
	...
	private function displaycheckout()
	{
		$modconfirm	= '';
		$modstatus = '';
		$this->_html .= "<link  media='screen' rel='stylesheet' href='../modules/unionpay/css/vtcss3.css'><div class='wap'>";
		$this->_html .= "<header class='headerTop'><div class='vt3logo'><img src='../modules/unionpay/img/vtLogo.png'></div>";
		$this->_html .= "<div class='slogan'><p>Sell to China,Paid via UnionPay Enabled by VTPayment</p></div><div class='header_btn'><ul><li>";
		...
		$this->_html .= " worldwide using all the UnionPay credit & debit cards.</h3><ul><li><img src='../modules/unionpay/img/unpay.png'>";
	...
	private function getValue($info)
	{
		return Tools::getValue($info);
	}
	private function displayForm()
	{
		$modcilpay = 'UnionPay Configuration';
		...
		$this->_html .= "<td class='vttd'  style='text-align: left; padding-left:20px;color:#000' >";
		$this->_html .= "<input type='text' name='SECURE_HASH_SECRET' value='{$mod_client_value_secure_hash_secret}' style='width: 300px;' /></td></tr>";
